 ...
 */
public class TestCase8_bi_1 extends AbstractTestCase {
	@Before
	public void before() {
		execute(new IDASSessionCallback() {
			public Object doInSession(IDASSession session) throws Throwable {
				List<DataObject> many1s = session.query(DASManager.createCriteria("case8_bi_1.Many1"));
				for (DataObject many1 : many1s) {
					session.deleteEntity(many1);
				}

				List<DataObject> many2s = session.query(DASManager.createCriteria("case8_bi_1.Many2"));
	...
	public void testInsert() {
		final DataObject many1_1 = ExtendedDataFactory.eINSTANCE.create("case8_bi_1.Many1");
		many1_1.set("id", "many1_1");
		many1_1.set("name", "many1_1_name");

		final DataObject many1_2 = ExtendedDataFactory.eINSTANCE.create("case8_bi_1.Many1");
		many1_2.set("id", "many1_2");
		many1_2.set("name", "many1_2_name");

		final DataObject many2_1 = ExtendedDataFactory.eINSTANCE.create("case8_bi_1.Many2");
		many2_1.set("id", "many2_1");
		many2_1.set("name", "many2_1_name");

		final DataObject many2_2 = ExtendedDataFactory.eINSTANCE.create("case8_bi_1.Many2");
		...
		DataObject queryRet = (DataObject) execute(new IDASSessionCallback() {
			public Object doInSession(IDASSession session) throws Throwable {
				return session.query(DASManager.createCriteria("case8_bi_1.Many2")).get(0);
			}
		});
		Assert.assertEquals(0, queryRet.getList("many1").size());
		detachedSession(queryRet);

		queryRet = (DataObject) execute(new IDASSessionCallback() {
			public Object doInSession(IDASSession session) throws Throwable {
				DataObject queryRet = session.query(DASManager.createCriteria("case8_bi_1.Many2")).get(0);
				queryRet.get("many1[1]/id");
				return queryRet;
			}
		});
		Assert.assertEquals(2, queryRet.getList("many1").size());
		detachedSession(queryRet);
	...
	public void testUpdate() {
		...
		final DataObject many2_3 = ExtendedDataFactory.eINSTANCE.create("case8_bi_1.Many2");
		...
		DataObject queryRet = (DataObject) execute(new IDASSessionCallback() {
			public Object doInSession(IDASSession session) throws Throwable {
				IDASCriteria criteria = DASManager.createCriteria("case8_bi_1.Many1");
