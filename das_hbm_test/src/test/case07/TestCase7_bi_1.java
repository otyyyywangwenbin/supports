...
public class TestCase7_bi_1 extends AbstractTestCase {
	@Before
	public void before() {
		execute(new IDASSessionCallback() {
			public Object doInSession(IDASSession session) throws Throwable {
				session.delete(DASManager.createCriteria("case7_bi_1.Many"));
				session.delete(DASManager.createCriteria("case7_bi_1.One"));
				return null;
			}
		});
	}
	...
	public void testInsert() {
		final DataObject one = ExtendedDataFactory.eINSTANCE.create("case7_bi_1.One");
		one.set("id", "one_id_01");
		one.set("name", "one_name_01");

		final DataObject many1 = ExtendedDataFactory.eINSTANCE.create("case7_bi_1.Many");
		many1.set("id", "many_id_01");
		many1.set("name", "many_name_01");
		final DataObject many2 = ExtendedDataFactory.eINSTANCE.create("case7_bi_1.Many");
		...
		DataObject queryOne = (DataObject) execute(new IDASSessionCallback() {
			public Object doInSession(IDASSession session) throws Throwable {
				DataObject queryOne = session.query(DASManager.createCriteria("case7_bi_1.One")).get(0);
				return queryOne;
			}
		});
		// NOTE 脱离session后的延迟加载对象必须从原有List复制一遍, 以避免出现SessionClosed的异常
		Assert.assertEquals(0, queryOne.getList("many").size());
		...
		// NOTE queryOne == queryOne/many[1]/one
		Assert.assertEquals(true, queryOne == queryOne.get("many[1]/one"));
	...
	public void testDelete() {
		...
		int count = (int) execute(new IDASSessionCallback() {
			public Object doInSession(IDASSession session) throws Throwable {
				return session.count(DASManager.createCriteria("case7_bi_1.One"));
			}
		});
		...
		count = (int) execute(new IDASSessionCallback() {
			public Object doInSession(IDASSession session) throws Throwable {
				return session.count(DASManager.createCriteria("case7_bi_1.Many"));
			}
		});
		Assert.assertEquals(0, count);
	}
