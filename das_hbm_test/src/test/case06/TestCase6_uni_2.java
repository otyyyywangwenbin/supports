...
public class TestCase6_uni_2 extends AbstractTestCase {
	@Before
	public void before() {
		execute(new IDASSessionCallback() {
			public Object doInSession(IDASSession session) throws Throwable {
				session.delete(DASManager.createCriteria("case6_uni_2.One1"));
				session.delete(DASManager.createCriteria("case6_uni_2.One2"));
				return null;
			}
		});
	}
	...
	public void testInsert() {
		final DataObject one2 = ExtendedDataFactory.eINSTANCE.create("case6_uni_2.One2");
		one2.set("id", "one2_id_01");
		one2.set("name", "one2_name_01");
		final DataObject one1 = ExtendedDataFactory.eINSTANCE.create("case6_uni_2.One1");
		...
		final DataObject queryRet = ExtendedDataFactory.eINSTANCE.create("case6_uni_2.One1");
	...
	//	one1.set("id", "one1_id_01");
		...
		int count = (Integer) execute(new IDASSessionCallback() {
			public Object doInSession(IDASSession session) throws Throwable {
				return session.count(DASManager.createCriteria("case6_uni_2.One1"));
			}
		});
