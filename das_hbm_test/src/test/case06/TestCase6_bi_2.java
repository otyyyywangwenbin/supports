...
public class TestCase6_bi_2 extends AbstractTestCase {
	@Before
	public void before() {
		execute(new IDASSessionCallback() {
			public Object doInSession(IDASSession session) throws Throwable {
				session.delete(DASManager.createCriteria("case6_bi_2.One1"));
				session.delete(DASManager.createCriteria("case6_bi_2.One2"));
				return null;
			}
		});
	}
	...
	public void testInsert() {
		final DataObject one2 = ExtendedDataFactory.eINSTANCE.create("case6_bi_2.One2");
		one2.set("id", "one2_id_01");
		one2.set("name", "one2_name_01");
		final DataObject one1 = ExtendedDataFactory.eINSTANCE.create("case6_bi_2.One1");
		...
		final DataObject queryRet = ExtendedDataFactory.eINSTANCE.create("case6_bi_2.One1");
		queryRet.set("id", "one2_id_01");
		execute(new IDASSessionCallback() {
			public Object doInSession(IDASSession session) throws Throwable {
		...
		//NOTE queryRet/one2/one1 != queryRet，只能做到有值
		Assert.assertEquals(true, queryRet.get("one2/one1") != null);

		DataObject queryRet2 = (DataObject) execute(new IDASSessionCallback() {
			public Object doInSession(IDASSession session) throws Throwable {
				return session.query(DASManager.createCriteria("case6_bi_2.One2")).get(0);
			}
		});
		//NOTE queryRet2/one1/one2 == queryRet2
	...
	public void testDelete() {
		...
		int count = (Integer) execute(new IDASSessionCallback() {
			public Object doInSession(IDASSession session) throws Throwable {
				return session.count(DASManager.createCriteria("case6_bi_2.One1"));
			}
		});
